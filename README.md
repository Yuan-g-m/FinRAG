...
FinRAG 是一个面向金融场景的检索增强生成（RAG）问答系统，覆盖股票、基金、债券、银行、保险五个主题。系统以 `Milvus` 作为向量检索引擎，结合 `BGE-M3` 稠密 + 稀疏混合检索与 `BGE-Reranker` 重排序，并引入「父块 + 子块」分层切分提升上下文完整性；同时以 `MySQL + BM25` 构建 FAQ 快速通道、`Redis` 承担高频缓存，再由 `DeepSeek` 兼容接口完成多策略生成。前端支持流式回答、思考过程可视化、多会话并行与归档管理。

## 核心能力

- **混合检索 + 重排序**：`BGE-M3` 双路向量召回，`BGE-Reranker` 对候选结果重排，兼顾语义与关键词匹配。
- **分层文档切分**：父块（1200）+ 子块（300）两级切分，检索用子块、回填用父块，保留更完整上下文。
- **多策略生成**：支持直接检索、HyDE、子查询、回溯问题四种策略，由意图分类与策略选择器按需调度。
- **FAQ 快速通道**：`MySQL + BM25 + Redis` 命中即返回，高频问题绕开重型检索链路，降低延迟与成本。
- **金融领域意图识别**：区分「通用知识」与「专业咨询」，通用问题直答，专业问题进入 RAG 流程。
- **多格式文档解析**：支持 Markdown、PDF、PPT、图片（OCR）等金融文档入库。
- **流式交互体验**：WebSocket 逐字输出与思考过程展示，支持多会话并行、归档/回收站与明暗主题切换。
- **可替换模型底座**：兼容 OpenAI 风格接口，通过 `LLM_BASE_URL` 切换不同 LLM 服务。
