# 该脚本是 FinRAG 金融领域 RAG 的集成问答主入口，在 RAG V1 基础上增加了：历史对话 + 流式输出.
# 当前金融版统一使用 rag_qa/core/new_rag_system.py，并在 rag_qa/__init__.py 中完成导入切换.
...
# 导入 OpenAI 客户端，用于调用 DeepSeek 的 OpenAI 兼容接口
from openai import OpenAI
# 导入时间库，用于记录处理时间
import time
import pymysql      # 这个库是Python操作MySQL的库, 用于数据库操作的异常捕获, 增删改查操作等.
import uuid         # 生成唯一会话ID, 标识: 用户会话.
import threading    # 线程锁, 用于保护共享 MySQL 连接在多线程并发下的数据安全.
...
class IntegratedQASystem:
    ...
    def __init__(self):
        ...
        self.db_lock = threading.Lock()  # 共享 MySQL 连接的线程锁
        self.redis_client = RedisClient()
        self.bm25_search = BM25Search(self.redis_client, self.mysql_client)
        # 3. 初始化大语言模型 -> LLM客户端.
        try:
            # 初始化 OpenAI 客户端，连接 DeepSeek API
        ...
        self.rag_system = RAGSystem(self.vector_store, self.call_llm)  # 创建 RAG 系统实例 -> 包括: 检索和生成.
    ...
    def init_conversation_table(self):
        ...
        try:
            ...
            # 2. 兼容旧表: 若 archived 字段不存在则自动添加, 用于归档/回收站功能.
            self.mysql_client.cursor.execute("SHOW COLUMNS FROM conversations LIKE 'archived'")
            if not self.mysql_client.cursor.fetchone():
                self.mysql_client.cursor.execute("""
                    ALTER TABLE conversations
                    ADD COLUMN archived TINYINT(1) NOT NULL DEFAULT 0
                """)
                self.mysql_client.connection.commit()            # 记录表初始化成功的日志
    ...
    def call_llm(self, prompt):
        """
        函数功能: 调用 DeepSeek API 生成答案 -> 流式输出
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "你是 FinRAG 金融智能助手，请基于金融知识严谨、清晰地回答问题，涉及投资建议时提示仅供参考。"},  # 系统提示
            ...
            # 作为生成器 yield 错误信息, 确保错误能被上层捕获并展示给用户.
    ...
    # todo 2.5 (优化5) 获取会话历史的方法: 对外提供获取指定会话完整历史的接口.
    def get_session_history(self, session_id):
        """
        函数功能: 从MySQL获取会话完整历史(前端展示用, 不再截断为最近5轮).
        :param session_id: 会话唯一标识(字符串)
        :return: 列表, 按时间正序返回全部对话记录.
        """
        try:
            with self.db_lock:
                self.mysql_client.cursor.execute("""
    ...
    def list_sessions(self, archived=False):
        """
        函数功能: 获取会话列表，用于前端侧边栏和回收站展示.
        :param archived: False -> 获取活跃会话; True -> 获取已归档会话.
        :return: 列表, 每项包含 session_id, turn_count, last_time, title(首条问题).
        """
    ...
    def set_session_archived(self, session_id: str, archived: bool) -> bool:
        ...
        try:
            ...
            self.logger.error(f"更新会话归档状态失败: {e}")
    ...
    def query(self, query, source_filter=None, session_id=None):
        ...
        # 2.获取最近5轮对话历史用于构造上下文, 若无 session_id 则返回空列表.
        history = self._fetch_recent_history(session_id) if session_id else []
        ...
        if answer:
            ...
            # 逐字返回答案, 与 RAG 流式保持一致, 保证前端纯 WebSocket 场景下的打字机体验
            for char in answer:
                yield char, False
            yield "", True
        elif need_rag:
...
def main():
    ...
    # 打印支持的金融领域
    print(f"支持的金融领域：{qa_system.config.VALID_SOURCES}")
    # 提示用户输入查询或退出
    ...
    try:
        while True:
            ...
            if query.lower() == "exit":
                ...
                # 退出循环
                break
            # 5. 获取用户输入的金融领域过滤 -> 允许用户指定领域, 无效则忽略.
            # 5.1 接收用户录入的金融领域
            source_filter = input(f"请输入金融领域 ({'/'.join(qa_system.config.VALID_SOURCES)}) (直接回车默认不过滤): ").strip()
            # 5.2 判断用户录入的 金融领域 是否有效.
            if source_filter and source_filter not in qa_system.config.VALID_SOURCES:
                # 如果金融领域过滤无效，记录警告日志
                logger.warning(f"无效的金融领域 '{source_filter}'，将不过滤")
                # 设置为空，忽略过滤
                source_filter = None
            # 6. 打印答案提示
            print("\n答案: ", end="", flush=True)
