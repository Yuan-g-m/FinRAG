...
class VectorStore:
    ...
    def __init__(
        self,
        collection_name=conf.MILVUS_COLLECTION_NAME,        # Milvus集合名 -> 类似于SQL的表名, 这里我们叫: finrag_final
        ...
        # 4.1 拼接重排序模型的本地路径. 优先使用 ASCII 模型目录，避免 sentencepiece 在 Windows 无法读取中文路径.
        models_dir = os.getenv('FINRAG_MODEL_ROOT') or os.path.join(rag_qa_path, 'models')
        if not os.path.isdir(models_dir):
            models_dir = os.path.join(rag_qa_path, 'models')
        reranker_path = os.path.join(models_dir, 'bge-reranker-large')
        ...
        # 5.1 拼接模型文件的本地路径. 即: models/bge-m3
        m3_path = os.path.join(models_dir, 'bge-m3')
        ...
        # 7. 确保目标数据库存在 -> Milvus 不会随连接自动建库.
        self._ensure_database()

        # 8. 初始化Milvus客户端, 建立: Milvus向量库连接.
        # uri格式: http://主机地址:端口号, db_name 指定要连接的数据库.
        self.client = MilvusClient(uri=f"http://{self.host}:{self.port}", db_name=self.database)

        # 9. 调用'私有'方法 -> 创建新集合(若不存在) 或 加载已有集合(若存在)
        self._create_or_load_collection()


    def _ensure_database(self):
        """创建 Milvus 数据库（若不存在）。"""
        admin_client = MilvusClient(uri=f"http://{self.host}:{self.port}")
        try:
            if self.database not in admin_client.list_databases():
                admin_client.create_database(self.database)
                self.logger.info(f"已创建 Milvus 数据库 {self.database}")
            else:
                self.logger.info(f"已加载 Milvus 数据库 {self.database}")
        finally:
            admin_client.close()
    ...
    def add_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            data.append({
                ...
                "source": doc.metadata.get("source", "unknown"),        # 金融领域(从元数据获取, 例如: 股票)
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        :param query: 用户查询文本, 例如: '股票开户需要哪些材料'
        :param k: 混合检索返回的TopK子块数量
        :param source_filter: 金融领域过滤条件, 例如: 股票, 仅检索该领域的文档, 默认None不过滤.
        ...
        # 4. 构建检索过滤表达式: 按金融领域过滤 -> 若source_filter存在.
...
if __name__ == '__main__':
    ...
    # 3.混合检索 + 结果重排, 例如: 查询'股票开户需要哪些材料', 过滤股票领域文档.
    # query = '什么是基金定投'
    query = '股票开户需要哪些材料'
    # query = '债券收益率受哪些因素影响'
    results = vector_store.hybrid_search_with_rerank(query, source_filter='股票')
